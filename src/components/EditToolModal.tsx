...
import { TOOL_CATEGORIES, ToolCategory, Tool, UpdateToolData } from '@/hooks/useTools';
...
interface EditToolModalProps {
  ...
  onClose: () => void;
  tool: Tool | null;
  onSubmit: (id: string, data: UpdateToolData) => Promise<boolean>;
}

export function EditToolModal({ isOpen, onClose, tool, onSubmit }: EditToolModalProps) {
  ...
  const handleSubmit = async () => {
    if (!tool || !name.trim() || !url.trim()) return;
    setSubmitting(true);
    const success = await onSubmit(tool.id, { name, url, description, category, imageUrl: imageUrl || undefined, isFeatured });
    setSubmitting(false);
    if (success) onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md rounded-2xl">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl tracking-wider">Editar Ferramenta</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input value={url} onChange={e => setUrl(e.target.value)} className="h-11 pl-10 rounded-xl" />
            </div>
          </div>

          <div className="space-y-2">
            <Label>Descrição</Label>
            <Textarea value={description} onChange={e => setDescription(e.target.value)} className="rounded-xl resize-y min-h-[80px]" />
          </div>
